...
public interface ResResrvQuery {
    //      예약접수 전체 리스트 출력
	public String selectAllReserv = "select T2.* from(select rownum rnum, T1.* from(select res_Id, customer_Id,res_Reserv_Date,res_Reserv_Fee from res_Reserv)T1)T2 where rnum between ? and ?";
    //      레스토랑별 예약접수 리스트 출력
	public String selectByResReserv = "select T2.* from(select rownum rnum, T1.* from(select  res_Id, customer_Id,res_Reserv_Date,res_Reserv_Fee from res_Reserv from res_Reserv where res_Id=?)T1)T2 where rnum between ? and ?";
    //      월별 범위안 예약접수 리스트 출력
    ...
    public String delete="delete from res_Reserv where res_Resrv_Id=?";
     //  예약번호별 예약정보 조회
    public String selectByResReservId ="select  res_Id, customer_Id,res_Reserv_Date,res_Reserv_Fee from res_Reserv from res_Reserv where res_Reserv_id=?";
    //   레스토랑 예약 count조회
